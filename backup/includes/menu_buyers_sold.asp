...
<table width="178" border="0" cellpadding="0" cellspacing="0" style="height: 100%;">
  ...
  <%
	dim connMenu2, rsMenu2, sqlM2
	set connMenu2 = new clsConnection
	set rsMenu2 = server.CreateObject("adodb.recordset")
	
	sqlM2 = "select pro_id, pro_name from tb_property where pro_status = true and pro_number = 2 ORDER BY pro_name ASC"
	rsMenu2.open sqlM2, connMenu2.conn,3 
	if not (rsMenu2.bof and rsMenu2.eof) then
%>
  <tr> 
    <td><a href="portfolio.asp"><img src="images/lnav_ttl_soldProperties.gif" width="179" height="35" border="0"></a></td>
  </tr>
  <% while not rsMenu2.eof %>
  <tr> 
    <td class="lnavSold">
	  <% if trim(request.QueryString("id")) = cstr(rsMenu2("pro_id")) then response.write "<b>" %>
	<a href="portfolioDetails.asp?id=<%= rsMenu2("pro_id") %>"><%= rsMenu2("pro_name") %></a>
	  <% if trim(request.QueryString("id")) = cstr(rsMenu2("pro_id")) then response.write "</b>" %>
	</td>
  </tr>
  <% 'if trim(request.QueryString("id")) = cstr(rsMenu2("pro_id")) then response.write menuProperty(trim(request.QueryString("id"))) %>
  <%
	rsMenu2.movenext
	wend
	end if
	rsMenu2.close
	set connMenu2 = nothing
%>

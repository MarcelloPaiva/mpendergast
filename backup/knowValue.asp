...
<%
Dim rsKValue__MMColParam
rsKValue__MMColParam = "true"
If (Request("MM_EmptyValue") <> "") Then 
  rsKValue__MMColParam = Request("MM_EmptyValue")
End If
%>
<%
Dim rsKValue
Dim rsKValue_numRows

Set rsKValue = Server.CreateObject("ADODB.Recordset")
rsKValue.ActiveConnection = MM_clsConnection_STRING
rsKValue.Source = "SELECT top 1 st.st_desc, st.st_footer,st.st_image, st.st_title  FROM tb_sec_text st  inner join tb_section s ON s.sec_id = st.sec_id  WHERE st.st_status = " + Replace(rsKValue__MMColParam, "'", "''") + " and s.sec_code = "SE003"  ORDER BY st.st_id DESC"
rsKValue.CursorType = 0
rsKValue.CursorLocation = 2
rsKValue.LockType = 1
rsKValue.Open()
          ...
          <td valign="top" bgcolor="#FFFFFF"> 
            ...
            <div id="content"> 
              <table width="100%" border="0" cellspacing="0" cellpadding="0">
                ...
                <tr bgcolor="#FFFFFF"> 
                  <td>&nbsp;</td>
                  <td colspan="2"> <% If Not rsKValue.EOF Or Not rsKValue.BOF Then %>
                    <table width="100%" border="0" cellpadding="2" cellspacing="3">
                      <tr> 
                        <td colspan="2" class="cntHdr01"><%=(rsKValue.Fields.Item("st_title").Value)%></td>
                      </tr>
                      <tr valign="top"> 
                        <td width="83%"> <p><%=(rsKValue.Fields.Item("st_desc").Value)%></p>
                          <p><%=(rsKValue.Fields.Item("st_footer").Value)%></p></td>
                        <td width="17%" align="right"><% if (rsKValue.Fields.Item("st_image").Value) <> "" then%> <img src="imagesDB/<%=(rsKValue.Fields.Item("st_image").Value)%>"> 
                          <% end if %></td>
                      </tr>
                    </table>
                    <% End If ' end Not rsKValue.EOF Or NOT rsKValue.BOF %> </td>
                </tr>
                <tr bgcolor="#FFFFFF"> 
                  <td>&nbsp;</td>
                  <td colspan="2">&nbsp;</td>

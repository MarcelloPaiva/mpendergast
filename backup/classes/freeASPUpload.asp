...
Class FreeASPUpload
	...
	Public Sub Save(path)
		...
		if not uploadedYet then Upload(path)
	...
	Private Sub Upload(path)
		Dim nCurPos, nDataBoundPos, nLastSepPos
		Dim nPosFile, nPosBound
		Dim sFieldName, osPathSep, auxStr
		dim intFileName, fileTemp, i, fs
		...
		Do Until nDataBoundPos = nLastSepPos
			...
			If nPosFile <> 0 And  nPosFile < nPosBound Then
				...
				'Add by Danilo Freitas - danilo.freitas@terra.com.br
				'Verify if file exist in folder
				fileTemp = Right(auxStr, Len(auxStr)-InStrRev(auxStr, osPathSep))
				intFileName = fileTemp
				i = 1
				Set fs = Server.CreateObject("Scripting.FileSystemObject")
				while fs.FileExists(path & "\" & intFileName)
					i = i + 1
					intFileName = right("000" & i, 3) & fileTemp
				wend
				Set fs = Nothing
				
				oUploadFile.FileName = intFileName
